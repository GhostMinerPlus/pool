...
use super::{DataSlice, FileRequest};
...
pub async fn set_data(
    ...
    main::set_data::<dep::Dep>(dm, hm, ds).await
}
...
pub async fn get_data(
    ...
    main::get_data::<dep::Dep>(dm, hm, fr).await
}
...
pub async fn delete_data(
    ...
    main::delete_data::<dep::Dep>(dm, hm, fr).await
}
...
mod main {
    ...
    use crate::{
        err,
        server::{DataSlice, FileRequest},
    };

    use super::dep::AsDep;

    pub async fn set_data<H: AsDep>(
        dm: Arc<dyn AsDataManager>,
        hm: &HeaderMap,
        ds: DataSlice,
    ) -> err::Result<()> {
    ...
    pub async fn delete_data<H: AsDep>(
        dm: Arc<dyn AsDataManager>,
        hm: &HeaderMap,
        fr: FileRequest,
    ) -> err::Result<()> {
...
mod dep {
    ...
    pub trait AsDep {
        ...
        fn get_data(start: u64, size: u64, key: &str) -> err::Result<(String, u64)> {
            let mut f = fs::File::open(format!("files/{key}"))
                .map_err(|e| err::Error::Other(e.to_string()))?;
            let length = f
                .metadata()
                .map_err(|e| err::Error::Other(e.to_string()))?
